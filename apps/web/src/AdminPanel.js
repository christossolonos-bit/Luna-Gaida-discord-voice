...
export function AdminPanel() {
    ...
    const savePlan = async () => {
        ...
        try {
            ...
            throw new Error('Feature configuration must be valid JSON.');
        }
    ...
    const deletePlan = async () => {
        if (!editingPlan?.id)
            return;
        if (!confirm(`Delete plan “${editingPlan.name}”? This cannot be undone.`))
            return;
        const result = await api(`/api/admin/plans/${editingPlan.id}`, { method: 'DELETE' });
        setEditingPlan(null);
        await reload();
